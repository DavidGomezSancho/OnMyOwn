...
/* ripv2_route_t * ipv4_route_create
 ...
 *   crear la ruta.
 */
ripv2_route_t * ripv2_route_create (ipv4_addr_t subnet, ipv4_addr_t mask, char* iface, ipv4_addr_t nxthop, int metric ,long int time){
	ripv2_route_t * route = (ripv2_route_t *) malloc(sizeof(struct ripv2_route));
	if(route!=NULL) {
    		memcpy(route->addr, subnet, IPv4_ADDR_SIZE);
    		memcpy(route->subnet_mask, mask, IPv4_ADDR_SIZE);
    		memcpy(route->iface, iface, IFACE_NAME_MAX_LENGTH);
    		memcpy(route->next_hop, nxthop, IPv4_ADDR_SIZE);
    		route->metrica = metric;
    		timerms_reset(&route->timer, time);
  	}
  	return route;
}
...
int ripv2_route_lookup ( ripv2_route_t * route, ipv4_addr_t addr ){
  ...
  memcpy(netmask, route->subnet_mask, IPv4_ADDR_SIZE);

  for (i = 0;i<IPv4_ADDR_SIZE;i++){
  	aux[i] = addr[i]&netmask[i];
  }
  memcpy(subnet, route->addr, IPv4_ADDR_SIZE);  
  if(memcmp(aux,subnet, IPv4_ADDR_SIZE) == 0){
...
void ripv2_route_print ( ripv2_route_t * route ){
  	if (route != NULL) {
    		...
    		ipv4_addr_str(route->addr, subnet_str);
    		char mask_str[IPv4_STR_MAX_LENGTH];
    		ipv4_addr_str(route->subnet_mask, mask_str);
    		char* iface_str = route->iface;
    		char nxhop_str[IPv4_STR_MAX_LENGTH];
    		ipv4_addr_str(route->next_hop, nxhop_str);
		long timeout=timerms_left(&(route->timer));
		int metric=route->metrica;
    		printf("%s/%s via %s dev %s timeout %ld metric %d", subnet_str, mask_str, nxhop_str, iface_str, timeout, metric);
 	}
}
...
ripv2_route_t* ripv2_route_read ( char* filename, int linenum, char * line ){
  	...
  	char mask_str[256];
  	char iface_name[256];
  	char nxt_hop_str[256];
	int timeout;
	int metric;

  	/* Parse line: Format "<subnet> <mask> <iface> <nxt_hop>\n" */
  	int params = sscanf(line, "%s %s %s %s %d %d\n", subnet_str, mask_str, iface_name, nxt_hop_str, &timeout, &metric);
  	if (params != 6) {
    		fprintf(stderr, "%s:%d: Invalid RIPv4 Route format: '%s' (%d params)\n",filename, linenum, line, params);
    		fprintf(stderr, "%s:%d: Format must be: <subnet> <mask> <iface> <nx_hop> <time> <metric>\n",filename, linenum);
    		return NULL;
  	}
	...
	if (err == -1) {
  	...
  	err = ipv4_str_addr(nxt_hop_str, next_hop);
  	if (err == -1) {
    		fprintf(stderr, "%s:%d: Invalid <nxt_hop> value: '%s'\n",filename, linenum, nxt_hop_str);
    		return NULL;
 	}

  	/* Initialize RIPv2 route timer */

  
  	/* Create new route with parsed parameters */
  	route = ripv2_route_create(subnet, mask, iface_name, next_hop, metric, timeout);
...
int ripv2_route_output ( ripv2_route_t * route, int header, FILE * out )
{
  ...
  if (header == 0) {
    err = fprintf(out, "# SubnetAddr  \tSubnetMask    \tIface  \tNext hop \tMetric \tTimeout \n");
    if (err < 0) {
      return -1;
    }
  }
  ...
  if (route != NULL) {
      ...
      ipv4_addr_str(route->next_hop, nxhop_str);
			int metric=route->metrica;
			long logout=timerms_left(&(route->timer));

      err = fprintf(out, "%-15s\t%-15s\t%s\t%-15s\t%d\t%ld\n",
		    subnet_str, mask_str, ifname, nxhop_str, metric, logout);
      if (err < 0) {
        return -1;
      }
  }
...
ripv2_route_table_t * ripv2_route_table_create()
{
  ...
  table = (ripv2_route_table_t *) malloc(sizeof(ripv2_route_table_t));
  if (table != NULL) {
    int i;
...
( ripv2_route_table_t * table, ipv4_addr_t subnet, ipv4_addr_t mask )
{
  ...
  if (table != NULL) {
    ...
    for (i=0; i<RIPv2_ROUTE_TABLE_SIZE; i++) {
      ...
      if (route_i != NULL) {
        int same_subnet = 
          (memcmp(route_i->addr, subnet, IPv4_ADDR_SIZE) == 0);
        int same_mask = 
