...
int main ( int argc, char *argv[])
{
	...
	if((open_ipv4("ipv4_config_client.txt", "ipv4_route_table_client.txt")) == -1){

		printf("Error en el cliente al inicializar\n");
		return -1;

	}

	if((send_ipv4(ip_destino, PROTOCOLO_SUPERIOR, (unsigned char *) datos,TAM_DATOS)) == -1){
